...
module.exports.sockets = {
  ...
  ***************************************************************************/
  onConnect: function(session, socket) {
	  ...
	  Participants.create({socketId: socketId}).exec(function(err, partcipant) {
		  ...
		  session.users[socketId] = partcipant;

		  // Persist the session
		  session.save();

		  // Send a message to the client with information about the new user
		  sails.sockets.emit(socketId, 'hello', partcipant);
		  ...
		  // between users.
		  Participants.subscribe(socket, partcipant, 'message');

		  // Get updates about users being created
		  Participants.watch(socket);

          Lessons.watch(socket);

		  // Get updates about rooms being created
		  Room.watch(socket);

		  // Publish this user creation event to every socket watching the Participants model via Participants.watch()
		  Participants.publishCreate(partcipant, socket);
  ...
  ***************************************************************************/
  onDisconnect: function(session, socket) {
	  ...
	  try {
		  ...
		  Participants.findBySocketId(sails.sockets.id(socket)).populate('rooms').exec(function(err, participant) {

              var id = participant[0].id;

			  // Destroy the user instance
			  Participants.destroy({id: id}).exec(function(){});
			  Participants.destroy({socketId: sails.sockets.id(socket)}).exec(function(){});

			  // Publish the destroy event to every socket subscribed to this user instance
			  Participants.publishDestroy(id, null)
		  });
	  } catch (e) {
...
//  origins: 'http://teachly.hushly.co.uk'
